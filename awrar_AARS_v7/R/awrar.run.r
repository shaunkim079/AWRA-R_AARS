...
awrar.run <- function(parameters,config,inputs,states=NULL,use_irrig=0,irrigParameters=NULL,irrigConfig=NULL,irrigInputs=NULL,irrigStates=NULL,outputOption=0,is_headwater=0,useDynMax=0,useDeadStorage=0){
  ...
  if(!is.numeric(useDeadStorage)){
    stop("useDeadStorage should be 0 or 1")
  }
  if(useDeadStorage!=1 & useDeadStorage!=0){
    stop("useDeadStorage should be 0 or 1")
  }
  
  if(!is.numeric(useDynMax)){
    stop("useDynMax should be 0 or 1")
  }
  if(useDynMax!=1 & useDynMax!=0){
    stop("useDynMax should be 0 or 1")
  }
  ...
  if(is_headwater==1 & length(which(inputs[,(get.constant("NINPUTSNONROUTING")+1)]>0))>0) stop("You said this wasn't a headwater but some inflow detected!")
  ...
  if(outputOption==0){
    ...
    names.states.nonrouting<-c("outflow","overbank.flow",
                               ...
                               "Salv",
                               "DepthToGW",
                               "alv.rainvol","alv.evapvol",
                               "QaquiferRiver.endTS",
                               "dead.storage.volume")
    if(is.null(dim(out$states.nonrouting))){
      names(out$states.nonrouting) <- names.states.nonrouting
      # names(out$states.nonrouting)[1:33] <- names.states.nonrouting
    } else {
      colnames(out$states.nonrouting)<-names.states.nonrouting
    ...
    if(any(is.na(out$states.nonrouting)) | any(is.infinite(out$states.nonrouting))){
      # which(is.na(out$outflow))[1]
      is.nas<-apply(out$states.nonrouting,2,function(x) which(is.na(x))[1])
      is.nas<-is.nas[!is.na(is.nas)]
      is.infinites<-apply(out$states.nonrouting,2,function(x) which(is.infinite(x))[1])
      is.nas<-is.nas[!is.na(is.nas)]
      is.infinites<-is.infinites[!is.na(is.infinites)]
      
      if(length(is.nas)>0){
        cat("Warning: NA in",names(is.nas[min(is.nas)==is.nas][1]),"- first ts NA:",is.nas[min(is.nas)==is.nas][1],"\n")
        # for(st in 1:length(is.nas)){
        #   cat("Warning: NA in",names(is.nas[st]),"- first ts NA:",is.nas[st],"\n")
        # }
      }
      if(length(is.infinites)>0){
        cat("Warning: Inf in",names(is.infinites[min(is.infinites)==is.infinites][1]),
            "- first ts Inf:",is.infinites[min(is.infinites)==is.infinites][1],"\n")
        # for(st in 1:length(is.infinites)){
        #   cat("Warning: Inf in",names(is.infinites[st]),"- first ts Inf:",is.infinites[st],"\n")
        # }
      }
