...
double actual_x(double dt,double x,double K)
{
	...
	actualx = x;
	if((actualx*K>=dt/2.0) & (K>0.0))		actualx = dt/2.0/K;
	if((K*(1-actualx)<=dt/2.0) & (K>0.0))  	actualx = 1.0-dt/2.0/K;
	if(K==0.0) 						actualx = 0.0;

	if(actualx<0.0) actualx=0.0;
	if(actualx>1.0) actualx=1.0;
	...
	)
{
    ...
    //instantaneous_outflow_prec =   *instantaneous_outflow;
		...
		else
		{
			...
			D = K*(1.0-actualx)+dt/2.0;
			c0 = -(K*actualx-dt/2.0)/D;
			c1 = (K*actualx+dt/2.0)/D;
			c2 = (K-K*actualx-dt/2.0)/D;
		...
		*sum_outflow 		+= 	*instantaneous_outflow*dt;
		...
		double routing_volume_tmp = *routing_volume;
		routing_volume_tmp 	+= 	(inflow-*instantaneous_outflow)*dt;
		if(routing_volume_tmp<0.0){
		  *instantaneous_outflow = (*routing_volume/dt)+inflow;
		  *routing_volume = 0.0;
		} else {
		  *routing_volume = routing_volume_tmp;
		}
